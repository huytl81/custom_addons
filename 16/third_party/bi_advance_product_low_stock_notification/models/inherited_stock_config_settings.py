...
class ResConfigSettings(models.TransientModel):
	...
	def set_values(self):
		...
		self.env['ir.config_parameter'].sudo().set_param('bi_advance_product_low_stock_notification.current_user', self.current_user)
	
	@api.model
	def get_values(self):
		res = super(ResConfigSettings, self).get_values()
		res.update({
			'current_user':self.env['ir.config_parameter'].sudo().get_param('bi_advance_product_low_stock_notification.current_user'),})
		return res
	...
	def action_list_products_(self):
		...
		res.user_company_name=res.create_uid.company_id.name
		test=self.company_id
		if res.id :
			products_dlt = [(2,dlt.id,0)for dlt in res.low_stock_products_ids]
			res.low_stock_products_ids = products_dlt
			user_company_id=res.create_uid.company_id
			...
			if res.notification_base == 'on_hand':
				if res.notification_products == 'for_all':
					...
					if res.notification_product_type == 'variant':	
						...
						for product in result.filtered(lambda x: x.company_id == test or x.company_id == user_company_id ):
							warehouse_list=list()
							name_att = ' '
							...
							res_id = self.env['stock.quant'].search([])
							for loop in res_id:
								if loop:
									if loop.product_id.name == product.name and loop.location_id.usage == 'internal':
										warehouse_list.append((str(loop.location_id.location_id.name or '')+'/'+str(loop.location_id.name or ''))+'='+(str(loop.available_quantity or ''))) 
								

							products_list.append([0,0,{'name':name_pro,
													'limit_quantity':res.min_quantity,
													'stock_quantity':product.qty_available,
													'location':[warehouse_list]}])
					else:
			...
			if res.low_stock_products_ids:
				if company:
					for company_is in company:
						...
						template_id = self.env['ir.model.data']._xmlid_lookup('bi_advance_product_low_stock_notification.low_stock_email_template')[2]
...
class low_stock_product(models.TransientModel):
	...
	location=fields.Char(string='Which location')
