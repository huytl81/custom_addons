...
class SaleOrderAPI(http.Controller):
    ...
    def list_sale_order(self, page=1):
        ...
        for item in items:
            ...
            obj = {
                ...
                'reason_cancel': '' if item.cancel_reason_for_customer == False else item.cancel_reason_for_customer,
    ...
    def cancel_sale_order(self, order_id=-1):
        ...
        if order:
            ...
            if order.online_order_state != 'draft':
                result['message'] = 'Không thể hủy do đơn hàng đã được xác nhận, liên hệ với nhân viên để được hỗ trợ'
                return result
            

            # if order.state ==
            # success = order.write({'customer_cancel': True})
            success = order.customer_cancel()
            if success:
                return {
                    'success': True,
    ...
    def get_sale_order(self, id=-1):
        ...
        result = {
            ...
            'reason_cancel': '' if order.cancel_reason_for_customer == False else order.cancel_reason_for_customer,
