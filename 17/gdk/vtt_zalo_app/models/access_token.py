...
class APIAccessToken(models.Model):
    ...
    def find_or_create_token(self, user_id=None, create=False):
        ...
        access_token = self.env["vtt.api.access.token"].sudo().search([("user_id", "=", user_id)], order="id DESC", limit=1)
        if access_token:
            access_token = access_token[0]
            # [2024-11-13] Comment bỏ check expire, coi nhu day la password cua user
            # if access_token.has_expired():
            #     access_token = None
        if not access_token and create:
            # token_expiry_date = datetime.now() + timedelta(seconds=int(self.env.ref(token_expiry_date_in).sudo().value))
            # token_expiry_date = datetime.now() + timedelta(days=30)
            # vals = {
            #     "user_id": user_id,
            #     "scope": "userinfo",
            ...
            #     "token": random_token(),
            # }
            vals = self.prepare_vals_to_create_token(user_id)
            access_token = self.env["vtt.api.access.token"].sudo().create(vals)
        if not access_token:
            return None
        return access_token.token

    def prepare_vals_to_create_token(self, user_id):
        token_expiry_date = datetime.now() + timedelta(days=30)
        vals = {
            "user_id": user_id,
            "scope": "userinfo",
