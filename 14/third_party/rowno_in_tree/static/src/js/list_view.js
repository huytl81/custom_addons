...
ListRenderer.include({
	_getNumberOfCols: function () {
		...
		columns +=1;
		var isLineSection = false;
		this.columns.forEach(function(item, index, arr){ if(item && item.attrs && ["section_and_note_text", "section_and_note_one2many"].includes(item.attrs.widget)) isLineSection = true; });
		columns = isLineSection ? columns -= 1 : columns;
		return columns;
    },
