...
class KsWooWebhooksConfiguration(models.Model):
    ...
    #     """
    #     Creates the webhook and also post it in woocommerce
    #     :param vals: creation data
    #     :return: rec
    #     """
    #     rec = super(KsWooWebhooksConfiguration, self).create(vals)
    #     for i in range(len(vals)):
    #         if not vals[i].get("ks_woo_id"):
    #             ks_instance_id = self.env['ks.woo.connector.instance'].search(
    #                 [('id', '=', vals[i].get('ks_instance_id'))])
    #             data = {
    #                 'name': vals[i].get('name'),
    #                 'status': rec[i].status if ks_instance_id.ks_instance_state == 'active' else 'disabled',
    #                 'topic': ".".join(vals[i].get('operations').split("_")) + 'd',
    #                 'delivery_url': rec[i].base_url
    #             }
    #             response_data = self.ks_create_webhook(ks_instance_id, data)
    #             if response_data:
    #                 rec[i].ks_woo_id = response_data.get("id")
    #             else:
