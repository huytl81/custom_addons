        ...
        if odoo_base_category:
            ...
            try:
                for id in hierarchy:
                    ...
                    if product_category.ks_product_category.ks_woo_category_id:
                        try:
                            category_id = product_category.ks_product_category.ks_woo_category_id
                            woo_category_data = self.ks_prepare_export_json_data(product_category,
                                                                                 product_category.ks_product_category)
...
class KsProductCategoryExtended(models.Model):
    ...
    def write(self, values):
        for rec in self:
            ks_woo_product_cat = self.env['ks.woo.product.category'].search([('ks_product_category', '=', rec.id), ('ks_wc_instance', '=', rec.ks_product_category.ks_wc_instance.id)])
            if ks_woo_product_cat.ks_woo_category_id:
                ks_woo_product_cat.update({'ks_sync_date': datetime.datetime.now()})
