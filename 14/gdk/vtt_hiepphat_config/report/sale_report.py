...
class SaleReport(models.Model):
    ...
    def _query(self, with_clause='', fields={}, groupby='', from_clause=''):
        ...
        select_ = """
            ...
            s.id as order_id,
            l.sale_state as sale_state
        """

        for field in fields.values():
            select_ += field

        mini_select_line_fields = ["id",
                                   "product_id",
                                   "product_uom_qty",
                                   "qty_delivered",
                                   "qty_invoiced",
        ...
        mini_select_returnline_fields = ["id",
                                         "product_id",
                                         "product_uom_qty",
                                         "qty_returned AS qty_delivered",
                                         "qty_invoiced",
                                         "product_uom_qty - qty_invoiced AS qty_to_invoice",
                                         "-price_total",
                                         "-price_subtotal",
                                         "-price_unit * (product_uom_qty - qty_invoiced) AS untaxed_amount_to_invoice",
                                         "-price_unit * qty_invoiced AS untaxed_amount_invoiced",
                                         "0.0 AS discount",
                                         "-price_unit",
                                         "ref_order_id",
                                         "product_uom",
                                         "'return' AS sale_state"]

        mini_from_ = """
            (SELECT %s FROM sale_order_line
            UNION
            SELECT %s FROM vtt_sale_order_return_line)
        """ % (', '.join(mini_select_line_fields), ', '.join(mini_select_returnline_fields))
    ...
    def old_query(self, with_clause='', fields={}, groupby='', from_clause=''):
        with_ = ("WITH %s" % with_clause) if with_clause else ""
