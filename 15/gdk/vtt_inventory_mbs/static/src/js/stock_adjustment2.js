/** @odoo-module alias=vtt_inventory_mbs.stock_adjustment2 **/
...
ListRenderer.include({	
    ...
    shUpdateCameraControl: async function () {    
        ...
        .then(function (result) {
      	  ...
      	  */
        	...
        	var $camSelect = $( QWeb.render('vtt_inventory_mbs.stock_adjustment.tree.cam_select', {'widget':self}) );
    ...
    _sh_barcode_scanner_load_widget_data: async function () {    
    	var self = this;
        const result = await session.rpc('/vtt_inventory_mbs/sh_barcode_scanner_get_widget_data', {
        });
...
//        self.sh_inven_adjt_barcode_scanner_warn_sound = result.sh_inven_adjt_barcode_scanner_warn_sound;        
    ...
    _renderView: function () {    	
        ...
        return this._super.apply(this, arguments).then(() => {            
            ...
            if (model == 'stock.quant' ){
            	var content_scanner = QWeb.render('vtt_inventory_mbs.stock_adjustment.tree.scan_feature', {
            		'widget':self,
    ...
    _on_change_sh_barcode_scanner_stock_quant_tree_input_barcode: async function (ev) {
        ...
        const result = await session.rpc('/vtt_inventory_mbs/sh_barcode_scanner_search_stock_quant_by_barcode', {
        	'barcode':barcode,
...
/*    		var message = _t(result.message);
    ...
    _on_click_js_cls_sh_barcode_scanner_stock_quant_tree_btn_apply: async function (ev) {
        ...
        ev.stopPropagation();
        var self = this;       
        const result = await session.rpc('/vtt_inventory_mbs/sh_barcode_scanner_stock_quant_tree_btn_apply', {
        });
        var error = false;
