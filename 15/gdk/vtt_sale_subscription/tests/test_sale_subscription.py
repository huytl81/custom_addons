...
class TestSubscription(TestSubscriptionCommon):
    ...
    def test_01_template(self):
        ...
        self.assertTrue(self.subscription.description, 'vtt_sale_subscription: description not copied on existing sale.subscription record')
        ...
        self.assertTrue(temp.description, 'vtt_sale_subscription: description not copied on new cached sale.subscription record')

    @mute_logger('odoo.addons.base.models.ir_model', 'odoo.models')
    def test_02_sale_order(self):
        """ Test sales order line copying for recurring products on confirm"""
        self.sale_order.action_confirm()
        self.assertTrue(len(self.subscription.recurring_invoice_line_ids.ids) == 1, 'vtt_sale_subscription: recurring_invoice_line_ids not created when confirming sale_order with recurring_product')
        self.assertEqual(self.sale_order.subscription_management, 'upsell', 'vtt_sale_subscription: so should be set to "upsell" if not specified otherwise')
    ...
    def test_05_sub_creation(self):
        ...
        self.assertEqual(len(self.sale_order_2.order_line.mapped('subscription_id')), 1, 'vtt_sale_subscription: subscription should be created on SO confirmation')
        self.assertEqual(self.sale_order_2.subscription_management, 'create', 'vtt_sale_subscription: subscription creation should set the SO to "create"')

        # Two product with different subscription template
        self.sale_order_3.action_confirm()
        self.assertEqual(len(self.sale_order_3.order_line.mapped('subscription_id')), 2, 'vtt_sale_subscription: Two different subscription should be created on SO confirmation')
        self.assertEqual(self.sale_order_3.subscription_management, 'create', 'vtt_sale_subscription: subscription creation should set the SO to "create"')

        # Two product with same subscription template
        self.sale_order_4.action_confirm()
        self.assertEqual(len(self.sale_order_4.order_line.mapped('subscription_id')), 1, 'vtt_sale_subscription: One subscription should be created on SO confirmation')
        self.assertEqual(self.sale_order_4.subscription_management, 'create', 'vtt_sale_subscription: subscription creation should set the SO to "create"')

        today = fields.Date.context_today(self.env.user)
        self.sale_order_5.action_confirm()
        self.assertEqual(self.sale_order_5.order_line.subscription_id.date, today + relativedelta(months=+1), 'vtt_sale_subscription: subscription creation should set the end date to : today + 1 month')
    ...
    def test_06_renewal(self):
        ...
        self.assertEqual(new_line.subscription_id, self.subscription, 'vtt_sale_subscription: SO lines added to renewal orders manually should have the correct subscription set on them')
        self.assertEqual(renewal_so.origin, self.subscription.code, 'vtt_sale_subscription: renewal order must have the "source document" set to the subscription code')
        self.assertEqual(renewal_so.subscription_management, 'renew', 'vtt_sale_subscription: renewal quotation generation is wrong')
        ...
        self.assertEqual(total_quantity, 7, 'vtt_sale_subscription: Renewal quantity is not equal to the SO quantity')
        self.assertEqual(renewal_so.subscription_management, 'renew', 'vtt_sale_subscription: so should be set to "renew" in the renewal process')
    ...
    def test_07_upsell_via_so(self):
        ...
        self.assertEqual(self.subscription, new_line.subscription_id,
                         '''vtt_sale_subscription: upsell line added to quote after '''
    ...
    def test_17_renew_archived_products(self):
        ...
        self.assertEqual(archived_product_ids, [self.product2.id], 'vtt_sale_subscription: Product 2 should be archived and recognized as such')
        self.assertTrue(archived_product_count == 1, 'vtt_sale_subscription: One product should be marked as archived')
        ...
        self.assertEqual(renewal_so.subscription_management, 'renew',
                         'vtt_sale_subscription: so should be set to "renew" in the renewal process')
        self.assertEqual(lines_renew[0][1], self.product.id, 'vtt_sale_subscription: First product should be TestProduct')
        self.assertEqual(lines_renew[0][2], 7,
                         'vtt_sale_subscription: Renewal quantity of replacement product already present should be equal to 10.')
        self.assertEqual(lines_renew[1][1], self.product3.id, 'vtt_sale_subscription: First product should be TestProduct')
        self.assertEqual(lines_renew[1][2], 10,
                         'vtt_sale_subscription: Renewal quantity of new replacement product should be equal to 3.')
