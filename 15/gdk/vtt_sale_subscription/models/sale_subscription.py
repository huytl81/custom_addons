...
class SaleSubscription(models.Model):
    ...
    def action_open_sales(self):
        ...
        return {
            ...
            "views": [[self.env.ref('vtt_sale_subscription.sale_order_view_tree_subscription').id, "tree"],
    ...
    def _track_subtype(self, init_values):
        ...
        if 'stage_id' in init_values:
            return self.env.ref('vtt_sale_subscription.subtype_stage_change')
    ...
    def _recurring_create_invoice(self, automatic=False, batch_size=20):
        ...
        invalid_payment_tag = self.env.ref('vtt_sale_subscription.subscription_invalid_payment', raise_if_not_found=False)
        if automatic:
                        ...
                        try:
                            ...
                            if tx is None or not tx.renewal_allowed:
                                ...
                                if close_subscription:
                                    template = self.env.ref('vtt_sale_subscription.email_payment_close')
                                    ...
                                    template = self.env.ref('vtt_sale_subscription.email_payment_reminder')
        ...
        if need_cron_trigger:
            if config['test_enable'] or config['test_file']:
                ...
                self.env.ref('vtt_sale_subscription.account_analytic_cron_for_invoice')._trigger()
    ...
    def send_success_mail(self, tx, invoice):
        ...
        template = self.env.ref('vtt_sale_subscription.email_payment_success')
...
class SaleSubscriptionTemplate(models.Model):
    ...
    invoice_mail_template_id = fields.Many2one(
        ...
        default=lambda self: self.env.ref('vtt_sale_subscription.mail_template_subscription_invoice', raise_if_not_found=False))
...
class SaleSubscriptionAlert(models.Model):
    ...
    def _compute_nextcall(self):
        cron = self.env.ref('vtt_sale_subscription.ir_cron_sale_subscription_update_kpi', raise_if_not_found=False)
        if not cron:
