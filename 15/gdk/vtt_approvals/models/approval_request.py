...
class ApprovalRequest(models.Model):
    ...
    def _compute_has_access_to_request(self):
        is_approval_user = self.env.user.has_group('vtt_approvals.group_approval_user')
        for request in self:
    ...
    def _get_user_approval_activities(self, user):
        domain = [
            ...
            ('res_id', 'in', self.ids),
            ('activity_type_id', '=', self.env.ref('vtt_approvals.mail_activity_data_approval').id),
            ('user_id', '=', user.id)
        ]
...
class ApprovalApprover(models.Model):
    ...
    def _create_activity(self):
        for approver in self:
            approver.request_id.activity_schedule(
                'vtt_approvals.mail_activity_data_approval',
    ...
    def _compute_can_edit(self):
        self.update({'can_edit': self.env.user.has_group('vtt_approvals.group_approval_user')})
