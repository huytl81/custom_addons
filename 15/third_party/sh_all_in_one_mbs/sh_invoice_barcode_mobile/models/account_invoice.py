...
class AccountMove(models.Model):
    ...
    def _get_tax_grouping_key_from_base_line(self, base_line, tax_vals):
        ...
        return {
            ...
            account.id,
            'currency_id':
            base_line.currency_id.id or self.currency_id.id,
            'analytic_tag_ids':
    ...
    sh_invoice_bm_is_cont_scan = fields.Char(
        ...
        default=default_sh_invoice_bm_is_cont_scan,
        readonly=True)
    ...
    def create(self, vals):
        ...
        if self.env.context.get('check_move_validity') != False:
            res = super(
                AccountMove,
                self).with_context(check_move_validity=False).create(vals)
        else:
    ...
    def _onchange_sh_invoice_barcode_mobile(self):
        ...
        # step 2 increaset product qty by 1 if product not in order line than create new order line.
            ...
            ).sh_invoice_barcode_mobile_type == "sh_qr_code":
                search_lines = self.invoice_line_ids.filtered(
            ...
            ).sh_invoice_barcode_mobile_type == "all":
                search_lines = self.invoice_line_ids.filtered(
                    lambda ol: ol.product_id.barcode == self.
                    sh_invoice_barcode_mobile or ol.product_id.default_code ==
                    self.sh_invoice_barcode_mobile or ol.product_id.sh_qr_code
                    == self.sh_invoice_barcode_mobile)
                domain = [
                    "|", "|",
            ...
            if search_lines:
                for line in search_lines:
                    ...
                    if self.env.company.sudo(
                    ).sh_invoice_bm_is_notify_on_success:
                ...
                if search_product:
                    ...
                    if self.move_type in ['out_invoice', 'out_refund']:
                        ...
                        if not account_id:
                            if self.env.company.sudo(
                            ).sh_invoice_bm_is_notify_on_fail:
                                message = _(
                    ...
                    if self.move_type in ['out_invoice', 'out_refund']:
                        if search_product.taxes_id:
                            invoice_line_val.update({
                                'tax_ids': [(6, 0,
                                             search_product.taxes_id.filtered(
                                                 lambda tax: tax.company_id ==
                                                 self.company_id).ids)]
                            })
                    elif self.move_type in ['in_invoice', 'in_refund']:
                        if search_product.supplier_taxes_id:
                            invoice_line_val.update({
                                'tax_ids':
                                [(6, 0,
                                  search_product.supplier_taxes_id.filtered(
                                      lambda tax: tax.company_id == self.
                                      company_id).ids)]
                            })
