...
class ProductProduct(models.Model):
    ...
    def _compute_vtt_qty_dict(self, lot_id, owner_id, package_id, from_date=False, to_date=False):
        ...
        #                                          orderby='id'))
        moves_in_res_past = {product.id: quantity for product, quantity in Move._read_group(domain_move_in_done, ['product_id'], ['quantity:sum'])}
        ...
        #                                           orderby='id'))
        moves_out_res_past = {product.id: quantity for product, quantity in Move._read_group(domain_move_out_done, ['product_id'], ['quantity:sum'])}
        ...
        #                                      orderby='id'))
        moves_in_from = {product.id: quantity for product, quantity in Move._read_group(domain_move_in_from, ['product_id'], ['quantity:sum'])}
        ...
        #                                       orderby='id'))
        moves_out_from = {product.id: quantity for product, quantity in Move._read_group(domain_move_out_from, ['product_id'], ['quantity:sum'])}
        ...
        for product in self.with_context(prefetch_fields=False):
            ...
            res[product_id] = {}
            qty_available = quants_res.get(origin_product_id, [0.0])[0] - moves_in_res_past.get(origin_product_id, 0.0) \
