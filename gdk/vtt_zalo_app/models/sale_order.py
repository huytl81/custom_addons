...
class SaleOrder(models.Model):
    ...
    def create(self, values):
        ...
        for input in values:
            if 'is_online_order' not in input.keys() or input['is_online_order'] != True:
                continue
            if 'delivery_address_id' in input.keys():
    ...
    # cap nhat trang thai van chuyen (phuc vu ca cron job + btn check trang thai)
    ...
    def apply_shipping_method(self):
        ...
        if carrier:
            ...
            if carrier and carrier.delivery_type == 'ghn_shipping':
                ...
                self._apply_ghn_shipping_method(carrier, ghn_from_district_id, ghn_from_ward_id, ghn_to_district_id,
                                                ghn_to_ward_code)
            else:
    ...
    def _apply_default_shipping_method(self, carrier, to_ward):
        shipping_fee = 0
        if to_ward and to_ward.shipping_allowed:
    ...
    def _apply_ghn_shipping_method(self, carrier, from_district_id, from_ward_id, to_district_id, to_ward_code):
        ...
        #     return
        #
        # from_district_id = warehouse.district_id.ghn_district_id
        # from_ward_id = warehouse.ward_id.ghn_ward_id
        #
        # to_district_id = self.partner_shipping_id.district_id.ghn_district_id
        # to_ward_code = self.partner_shipping_id.ward_id.ghn_ward_id
        #
        # if from_district_id < 1 or to_district_id < 1 or not from_ward_id or not to_ward_code:
        #     return
