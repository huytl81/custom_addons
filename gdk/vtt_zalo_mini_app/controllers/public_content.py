...
class PublicContentAPI(http.Controller):
    ...
    @http.route([
        ...
        '/image/<string:model>/<int:id>/<string:field>/<string:subfield>'
    ], type='http',
        auth="public")
    def vtt_source_image(self, model='ir.attachment', id=None, field='image_1024', subfield=None):
        odoo_model = 'ir.attachment'
        allowed_models = {
            'news': 'vtt.zalo.news',
            'product': 'product.template',
            ...
            'zalo_message': 'zalo.message.template',
            # 'zalo_send_messages_transaction': 'zalo.send.messages.transaction',
            # 'zalo_send_messages_broadcast_individual': 'zalo.send.messages.broadcast.individual'
        }
        ...
        if model == 'news' and field == 'image_1024':
            field = 'image'
        if model == 'company':  # Logic for company model
            if subfield == 'logo':  # Use subfield to specify logo
                field = 'logo'  # Assuming 'logo' is the field for the company logo
            elif subfield == 'bank_qr':  # Or if you want the QR
                field = 'bank_qr'
            else:  # Default to logo if no subfield is specified
                field = 'bank_qr'
        ...
        if model == 'member_card' and field == 'image_1024':
            field = 'icon'
        # if model == 'zalo_send_messages_transaction':
        #     if subfield:  # Nếu có subfield
        #         field = subfield  # Gán subfield cho field
        #     else:
        #         field = 'banner'  # Trường hợp không có subfield (ảnh bìa)
        # if model == 'zalo_send_messages_broadcast_individual':
        #     if subfield:  # Nếu có subfield
        #         field = subfield  # Gán subfield cho field
        #     else:
        #         field = 'banner'  # Trường hợp không có subfield (ảnh bìa)
