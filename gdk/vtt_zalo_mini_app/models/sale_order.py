...
class SaleOrder(models.Model):
    ...
    delivery_fee = fields.Float('Phí ship', compute='_compute_delivery_fee', store=True)
    ...
    cancel_from = fields.Selection([
        ('customer', 'Khách hàng'),
        ('employee', 'Nhân viên')
    ], string='Hủy từ', default=None)

    cancel_reason_for_customer = fields.Char('Lý do hủy gửi khách hàng')

    # thong tin ship
    shipper_name = fields.Char('Shipper')
    shipper_phone = fields.Char('SĐT shipper')
    actual_shipping_fee = fields.Float('Phí ship thực tế')
    ...
    #     #     order.custom_total = total  # lưu vào 1 field tự tạo trong model sale.order

    # custom_total = fields.Monetary(compute='_compute_total', string='Custom Total')
    ...
    def _onchange_change_partner_shipping(self):
        if self.delivery_address_id and self.is_online_order == True:
            ...
            # address = ''
            # if order.delivery_address_id and order.is_online_order == True:
            #     order.partner_shipping_id = order.delivery_address_id.id

            if not self.delivery_address_id or self.is_online_order != True:                
                return

            if self.delivery_address_id and self.delivery_address_id.street and self.is_online_order == True:
                self.partner_shipping_id = self.delivery_address_id.id
                ...
                # order.write({'full_text_delivery_address': address})
    ...
    #         if not order.delivery_address_id or order.is_online_order != True:                
    #             continue

    #         if order.delivery_address_id and order.delivery_address_id.street and order.is_online_order == True:
    #             order.partner_shipping_id = order.delivery_address_id.id

    #             if order.delivery_address_id and order.delivery_address_id.ward_id:
    #                 street = order.delivery_address_id.street
    #                 ward = order.delivery_address_id.ward_id
    #                 if ward.state_id and ward.district_id:
    #                     district = ward.district_id
    #                     state = ward.state_id
    #                     address = ', '.join((street, ward.name, district.name, state.name))
    #             # order.write({'full_text_delivery_address': address})
    #             order.full_text_delivery_address = address
    #         bb = ''
    ...
    def customer_confirm(self):
        ...
        # send notification cho NV
        sale_order_id = self.id  # The ID of your sale order
        ...
        notify_message = {
            ...
            'sticky': True,
            'links_u': [{
                'label': f'Đơn hàng {order_name}',
                'url': sale_order_url
            }]
        }
        channel_mess = f'Đã đặt hàng, mã đơn hàng: {order_name}'
        self._notify_to_sale_person(notify_message, channel_mess)  


        # send zns
        self.env['zalo.message.template'].sudo().send_zns_transaction_message(order=self, event='customer_confirmed')
    ...
    def action_delivering(self):
        ...
        if self.online_order_state not in ['done', 'cancel']:
            ...
            self.message_post(body="Chuyển trạng thái: Đang giao hàng", message_type='notification', subtype_xmlid='mail.mt_note')

            # send zns
            self.env['zalo.message.template'].sudo().send_zns_transaction_message(order=self, event='order_delivering')
    ...
    def action_done(self):
        if self.online_order_state != 'cancel':
            ...
            # send zns
            self.env['zalo.message.template'].sudo().send_zns_transaction_message(order=self, event='order_done')

    # ghi nhận việc thanh toán từ VNPAY hoặc NV chủ động xác nhận thanh toán
    def action_paid(self, vnpay_transaction_id=None):
        # Create invoice
        ...
        chatter_note = 'Đã ghi nhận thanh toán'
        if vnpay_transaction_id:
            chatter_note = f'Đã ghi nhận thanh toán qua VNPAY, mã giao dịch: {vnpay_transaction_id}'
            # self.vnpay_transaction_id = vnpay_transaction_id
            # send notification cho NV
            sale_order_id = self.id  # The ID of your sale order
            order_name = self.name
            base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
            ...
            channel_mess = f'Đơn hàng {order_name} đã được thanh toán qua VNPAY, mã giao dịch: {vnpay_transaction_id}'
            self._notify_to_sale_person(notify_message, channel_mess)   

        self.message_post(body=chatter_note, message_type='notification', subtype_xmlid='mail.mt_note')

        # send zns
        self.env['zalo.message.template'].sudo().send_zns_transaction_message(order=self, event='order_paid')
        return True
    ...
    def _create_invoice(self):
        ...
        for line in self.order_line:
            invoice_line_vals = {
                ...
                'sale_line_ids': [(6, 0, [line.id])],  # Link the invoice line to the sale order line
            }
            invoice_vals['invoice_line_ids'].append((0, 0, invoice_line_vals))
        invoice = self.env['account.move'].create(invoice_vals)
        return invoice

    def _get_custom_point_changes(self):
        """
    ...
    #
    # print("code_enabled_rule_ids: ", code_enabled_rule_ids.read())
    ...
    def _change_full_text_delivery_address(self):
        ...
        if self.delivery_address_id and self.delivery_address_id.ward_id:
            ...
            if ward.state_id and ward.district_id:
                ...
                state = ward.state_id
                address = ', '.join((street, ward.name, district.name, state.name))
        self.write({'full_text_delivery_address': address})
        # self.full_text_delivery_address = address   
    
    def customer_cancel(self):
        # [ ] Can bo sung thông báo cho NV, dựa trên cơ sở trạng thái KH đã xác nhận chưa, nếu đã xác nhận thì mới thông báo        
            # them gui thong bao nv o day        
        if self.online_order_state == 'draft': 
            self._action_cancel()                            
            if self.customer_confirmed == True:                
                self.cancel_from = 'customer'
                self.cancel_reason_for_customer = 'Khách hàng hủy đơn hàng'            
                self.message_post(body="Hủy đơn hàng", message_type='notification', subtype_xmlid='mail.mt_note')            
                ...
                channel_mess = f'Hủy đơn hàng: {order_name}'
                self._notify_to_sale_person(notify_message, channel_mess)   
    ...
    def action_abc(self):
        ...
        channel_mess = f'Hủy đơn hàng: {order_name}'
        self._notify_to_sale_person(notify_message, channel_mess)   
        # self.message_post(body="bbbbb", message_type='notification', subtype_xmlid='mail.mt_note')

    def action_cancel(self):
        if self.is_online_order:
            # Hủy đơn hàng online, action cancel là NV bấm nút hủy
            # bat wizard huy don hang
            return {
                'type': 'ir.actions.act_window',
                'res_model': 'cancel.online.order.wizard',
                'view_mode': 'form',
                'target': 'new',
                'context': {'default_cancel_reason': self.cancel_reason_for_customer},
            }
        else:
            # Quay lại quy trình hủy đơn hàng thông thg
    ...
    def _action_cancel(self):
        ...
        for coupon, changes in previously_confirmed._get_custom_point_changes().items():
            if coupon.program_type == 'loyalty' and coupon.program_id.auto_apply == True and coupon.partner_id.id == self.partner_id.id:
                coupon_id = coupon.id
        ...
        # send zns
        self.env['zalo.message.template'].sudo().send_zns_transaction_message(order=self, event='order_cancel')
    ...
    def _notify_to_sale_person(self, notify_message, message=None):        
            ...
            if channel:
                ...
                for p in members:                     
                    notifications.append((
                        p, 'simple_notification_1', notify_message
                    ))
