...
class LoyaltyCard(models.Model):
    ...
    # track_points = fields.Float('Track points', default=0, compute='_compute_track_points') #     

    @api.depends('level_points')
    def _compute_tier(self):
        if self.program_id and self.program_id.program_type != 'loyalty':
            return
        if self.program_id and self.program_id.loyalty_tier_ids and len(self.program_id.loyalty_tier_ids):
            ...
            self.tier_id = t
            if self.partner_id:
                self.partner_id.loyalty_tier_id = t
                self.partner_id.loyalty_points = self.points
        else:
            self.tier_id = False
