...
class MultiStepWizard(models.AbstractModel):
    ...
    multistep_state = fields.Selection(
        selection="_selection_multistep_state", default="start", required=True
    )

    allow_back = fields.Boolean(compute="_compute_allow_back")

    @api.depends("multistep_state")
    def _compute_allow_back(self):
        for record in self:
            record.allow_back = getattr(
                record, "state_previous_%s" % record.multistep_state, False
            )
    ...
    def open_next(self):
        state_method = getattr(self, "multistep_state_exit_{}".format(self.multistep_state), None)
        if state_method is None:
            raise NotImplementedError(
                "No method defined for state {}".format(self.multistep_state)
            )
        state_method()
        return self._reopen_self()

    def open_previous(self):
        state_method = getattr(self, "state_previous_{}".format(self.multistep_state), None)
        if state_method is None:
            raise NotImplementedError(
                "No method defined for state {}".format(self.multistep_state)
            )
