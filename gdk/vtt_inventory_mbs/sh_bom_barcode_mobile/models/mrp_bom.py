...
class MrpBom(models.Model):
    ...
    sh_bom_bm_is_cont_scan = fields.Char(
        ...
        default=default_sh_bom_bm_is_cont_scan,
        readonly=True)
    ...
    def _onchange_sh_bom_barcode_mobile(self):
        ...
        if self and self.sh_bom_barcode_mobile:
            ...
            ).sh_bom_barcode_mobile_type == "barcode":
            ...
            ).sh_bom_barcode_mobile_type == "int_ref":
                search_lines = self.bom_line_ids.filtered(
            ...
            ).sh_bom_barcode_mobile_type == "sh_qr_code":
                search_lines = self.bom_line_ids.filtered(
            ...
            ).sh_bom_barcode_mobile_type == "all":
                search_lines = self.bom_line_ids.filtered(
                    lambda ol: ol.product_id.barcode == self.
                    sh_bom_barcode_mobile or ol.product_id.default_code == self
                    .sh_bom_barcode_mobile or ol.product_id.sh_qr_code == self.
                    sh_bom_barcode_mobile)
                domain = [
                    "|",
                    "|",
            ...
            if search_lines:
                ...
                if search_product:
                    ...
                    if self.env.user.company_id.sudo(
                    ).sh_bom_bm_is_notify_on_success:
