...
class KsWooProductTemplate(models.Model):
    ...
    ks_woo_product_type = fields.Selection([('simple', 'Simple Product'), ('grouped', 'Grouped Product'),
                                           ...
                                           string='Woo Product Type', store=True, default="simple", help="Displays WooCommerce Product Type")
    ...
    ks_woo_product_id = fields.Integer('Woo Product ID',
                                       help=_("the record id of the particular record defied in the Connector"), readonly=True)
    ...
    ks_wc_tag_ids = fields.Many2many('ks.woo.product.tag', string='Tags',
                                     domain="[('ks_woo_tag_id', '!=', 0), "
                                            "('ks_wc_instance', '=', ks_wc_instance)]", help="Displays WooCommerce Tags")

    ks_wc_variant_ids = fields.One2many('ks.woo.product.variant', 'ks_wc_product_tmpl_id', string='Variants',
                                        readonly=True)

    ks_woo_rp_pricelist = fields.Many2one("product.pricelist.item", string="Regular Pricelist", help="Displays WooCommerce Regular Price")
    ks_woo_sp_pricelist = fields.Many2one("product.pricelist.item", string="Sale Pricelist", help=" Displays WooCommerce Sale Price")
    ...
    def ks_update_woo_regular_price(self):
        ...
        """
        for rec in self:
            rec.ks_woo_regular_price = str(self.env['product.pricelist.item'].search([('pricelist_id', '=', rec.ks_wc_instance.ks_woo_regular_pricelist.id),
                                                                                   ('product_id', '=', rec.ks_product_product.id)], limit=1).price) if self.env['product.pricelist.item'].search([('pricelist_id', '=', rec.ks_wc_instance.ks_woo_regular_pricelist.id),
                                                                                   ('product_id', '=', rec.ks_product_product.id)], limit=1).price else '0.0'
    ...
    def ks_update_woo_sale_price(self):
        ...
        """
        for rec in self:
            rec.ks_woo_sale_price = str(self.env['product.pricelist.item'].search([('pricelist_id', '=', rec.ks_wc_instance.ks_woo_sale_pricelist.id),
                                                                                   ('product_id', '=', rec.ks_product_product.id)], limit=1).price) if self.env['product.pricelist.item'].search([('pricelist_id', '=', rec.ks_wc_instance.ks_woo_sale_pricelist.id),
                                                                                   ('product_id', '=', rec.ks_product_product.id)], limit=1).price else '0.0'
                    ...
                    try:
                        if product_exist.ks_woo_product_type == product_json_data.get("type"):
                            ...
                            product_exist.ks_product_template.update({'weight': product_json_data.get('weight') or 0.0,
                                                                      "volume": float(product_json_data.get('dimensions').get('length') or 0.0) * float(
                product_json_data.get('dimensions').get('height') or 0.0) * float(
                product_json_data.get('dimensions').get('width') or 0.0),
                                                                      })
                            if instance.ks_sync_images:
                                self.env['ks.woo.product.images'].ks_update_images_for_odoo(product_json_data.get("images"),
                                                                                        product=product_exist.id)
                ...
                try:
                    ...
                    if product_data_response:
                        ...
                        self.env['ks.woo.product.images'].ks_update_images_for_odoo(product_data_response.get("images"),
                                                                                    product=self.id)
                        self.ks_wc_variant_ids.ks_manage_woo_product_variant_export(instance,
                                                                                    product_config=product_config)
    ...
    def ks_manage_category_to_export(self, instance, product_template):
        ...
        """
        layer_category = product_template.categ_id.ks_product_category.filtered(lambda x: x.ks_wc_instance.id == instance.id)
    ...
    def ks_manage_attributes_import(self, instance, attribute_json_data, main_product=False):
        ...
        for attr in attribute_json_data:
            ...
            odoo_attribute = self.env['ks.woo.product.attribute'].ks_manage_attribute_import(instance, woo_attr_json if woo_attr_json else attr)
            if main_product:
...
class KsProductTemplateInherit(models.Model):
    ...
    def ks_manage_template_images(self, woo_product, template):
        if template.ks_image_ids:
            for image in template.ks_image_ids:
                image_id = self.env['ks.woo.product.images'].ks_odoo_prepare_image_data(image, template_id=woo_product.id,
                                                                             variant_id=False)
    ...
    def write(self, values):
        for rec in self:
            ks_woo_product_temp = self.env['ks.woo.product.template'].search([('ks_product_template.id', '=', rec.id), ('ks_wc_instance.id', '=', rec.ks_product_template.ks_wc_instance.id)])
            if ks_woo_product_temp.ks_woo_product_id:
                ks_woo_product_temp.update({'ks_sync_date': datetime.datetime.now()})
